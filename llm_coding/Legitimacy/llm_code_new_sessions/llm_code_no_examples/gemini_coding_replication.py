#!/usr/bin/env python3
"""
Gemini Coding Replication Script - NO EXAMPLES VERSION
...
"""
    ...
    def run_replication(self, data_dir: str = "../data collection") -> pd.DataFrame:
        logger.info("Starting Gemini coding replication (NO EXAMPLES version)")
    ...
    def save_results(self, df: pd.DataFrame):
        ...
        csv_path = output_dir / "gemini_classifications_no_examples.csv"
        df.to_csv(csv_path, index=False)
        logger.info(f"Saved results to {csv_path}")

        try:
            dta_path = output_dir / "gemini_classifications_no_examples.dta"
...
def main():
    ...
    try:
        ...
        print(f"Results saved to results/gemini_classifications_no_examples.csv")
    except Exception as e:
