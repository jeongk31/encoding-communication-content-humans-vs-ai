...
Return ONLY the prompt template text, nothing else. Do not wrap it in quotes or code blocks."""
    ...
    def run_replication(self, data_dir: str = "../data collection", output_dir: str = None) -> pd.DataFrame:
        logger.info("Starting Gemini coding replication (PROMPT GENERATION version)")
    ...
    def save_results(self, df: pd.DataFrame):
        ...
        csv_path = self.output_dir / "gemini_classifications_no_examples.csv"
        df.to_csv(csv_path, index=False)
        logger.info(f"Saved results to {csv_path}")

        try:
            dta_path = self.output_dir / "gemini_classifications_no_examples.dta"
