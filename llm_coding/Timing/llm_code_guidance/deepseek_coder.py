...
def main():
    ...
    logger.info("Starting encoding with DeepSeek (WITH EXAMPLES)...")

    # Initialize LLM
    client_or_model, model_name = init_llm()
    logger.info(f"Initialized DeepSeek ({model_name})")
    ...
    # Save results
    output_file = os.path.join(RESULTS_DIR, "deepseek.csv")
    ...
    # Summary statistics
    print(f"\nENCODING SUMMARY (DEEPSEEK - WITH EXAMPLES)")
    print(f"{'=' * 50}")
