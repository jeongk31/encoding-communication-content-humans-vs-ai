#!/usr/bin/env python3
"""
GPT Coding Replication Script
Replicates the human coding exercise using OpenAI GPT API for the Promises and Partnerships experiment data.

This script processes all CSV files and Excel files in the Data directory and uses GPT to classify
...
class GPTCodingReplication:
    def __init__(self, api_key: str, test_mode: bool = False):
        """
        Initialize the GPT coding replication system.
...
"""
    ...
    def classify_message_with_gpt(self, message: str, context: str = "") -> Dict[str, Any]:
        """
        Use GPT to classify a message.
        ...
        try:
            ...
            logger.error(f"Error classifying message with GPT: {e}")
            return {
    ...
    def _extract_classification_from_text(self, text: str) -> Dict[str, Any]:
        """
        Extract classification from GPT's text response when JSON parsing fails.

        Args:
            text (str): GPT's response text
    ...
    def process_table_s1(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Processed dataframe with GPT classifications
        """
        ...
        for idx, row in df.iterrows():
            ...
            # Classify with GPT
            gpt_result = self.classify_message_with_gpt(message, context)
            ...
            result_row = {
                ...
                'ID': player_id,
                'Message': message,
                'Class': gpt_result['classification']
            }
    ...
    def run_full_replication(self, data_dir: str = "../Data") -> Dict[str, pd.DataFrame]:
        ...
        """
        logger.info("Starting full GPT coding replication")
    ...
    def save_results(self, results: Dict[str, pd.DataFrame]):
        ...
        if all_results:
            ...
            output_file = output_dir / "gpt_classifications.csv"
            combined_df.to_csv(output_file, index=False)
            logger.info(f"Saved all results to {output_file}")

def main():
    """
    Main function to run the GPT coding replication.
    """
    ...
    replicator = GPTCodingReplication(api_key, test_mode=test_mode)
    ...
    try:
        ...
        print("Results saved to results/gpt_classifications.csv")
