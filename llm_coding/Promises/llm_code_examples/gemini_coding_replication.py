...
class GeminiCodingReplication:
    def __init__(self, api_key: str, test_mode: bool = False):
        """
        Initialize the Gemini coding replication system.
...
"""
    ...
    def classify_message_with_gemini(self, message: str, context: str = "", max_retries: int = 5) -> Dict[str, Any]:
        """
        Use Gemini to classify a message with retry logic for rate limiting.
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    # Fallback parsing
                    result = self._extract_classification_from_text(response.text)
    ...
    def _extract_classification_from_text(self, text: str) -> Dict[str, Any]:
        """
        Extract classification from Gemini's text response when JSON parsing fails.

        Args:
            text (str): Gemini's response text
    ...
    def process_table_s1(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Processed dataframe with Gemini classifications
        """
    ...
    def run_full_replication(self, data_dir: str = "../Data") -> Dict[str, pd.DataFrame]:
        ...
        """
        logger.info("Starting full Gemini coding replication")
    ...
    def save_results(self, results: Dict[str, pd.DataFrame]):
        ...
        if all_results:
            ...
            output_file = output_dir / "gemini_classifications.csv"
...
def main():
    """
    Main function to run the Gemini coding replication.
    """
