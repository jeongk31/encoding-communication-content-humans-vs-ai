...
def generate_coding_prompt(model, model_name):
...
Return ONLY the prompt template text, nothing else. Do not wrap it in quotes or code blocks."""
    ...
    full_prompt = "You are a research assistant helping to create coding prompts for experimental data analysis.\n\n" + meta_prompt
...
def call_llm(client_or_model, model_name, prompt, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = client_or_model.generate_content(full_prompt)
...
def main():
    ...
    logger.info("Starting encoding with Gemini (prompt generation mode)...")
    ...
    prompt_file = os.path.join(RESULTS_DIR, "generated_prompt_gemini.txt")
    ...
    # Summary statistics
    print(f"\nENCODING SUMMARY (GEMINI - PROMPT GENERATION)")
    print(f"{'=' * 50}")
