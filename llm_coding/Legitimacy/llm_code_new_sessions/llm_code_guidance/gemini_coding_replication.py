#!/usr/bin/env python3
"""
Gemini Coding Replication Script - CONTEXT VERSION
...
"""
    ...
    def run_replication(self, data_dir: str = "../data collection") -> pd.DataFrame:
        logger.info("Starting Gemini coding replication (CONTEXT version)")
    ...
    def save_results(self, df: pd.DataFrame):
        ...
        csv_path = output_dir / "gemini_labeled_data.csv"
        df.to_csv(csv_path, index=False)
        logger.info(f"Saved results to {csv_path}")

        try:
            dta_path = output_dir / "gemini_labeled_data.dta"
...
def main():
    ...
    try:
        ...
        print(f"Results saved to results/gemini_labeled_data.csv")
    except Exception as e:
